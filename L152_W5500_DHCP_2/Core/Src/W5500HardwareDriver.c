...
void W5500Initialze(void)
{
  ...
  //reg_wizchip_spiburst_cbfunc(W5500ReadByte, W5500WriteByte);
...
void W5500WriteByte(unsigned char byte)
{
  ...
  while(!SPI_I2S_GetFlagStatus(W5500_SPI, SPI_I2S_FLAG_TXE));
  SPI_I2S_SendData(W5500_SPI, byte);
  //while (SPI_I2S_GetFlagStatus(W5500_SPI, SPI_I2S_FLAG_RXNE) == RESET);
  while(!SPI_I2S_GetFlagStatus(W5500_SPI, SPI_I2S_FLAG_RXNE)); //data received?
...
{
  //while (SPI_I2S_GetFlagStatus(W5500_SPI, SPI_I2S_FLAG_TXE) == RESET);
  while(!SPI_I2S_GetFlagStatus(W5500_SPI, SPI_I2S_FLAG_TXE));  //transmit buffer empty?
